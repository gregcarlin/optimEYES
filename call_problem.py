...
class CallProblemBuilder:
    def __init__(self, start_date: date, resident_availability: Mapping[str, Sequence[int]]) -> None:
        ...
        for resident, availability in resident_availability.items():
            ...
            self.problem.add_constraint(sum(self.day_vars[resident]) >= min_days_per_resident)
            self.problem.add_constraint(sum(self.day_vars[resident]) <= max_days_per_resident)

        # Ensure exactly one resident is assigned to each day
        for day in range(self.num_days):
            all_residents_for_day = [days_for_resident[day] for days_for_resident in self.day_vars.values()]
        ...
        for days_for_resident in self.day_vars.values():
            for i in range(len(days_for_resident) - 1):
                self.problem.add_constraint(days_for_resident[i] + days_for_resident[i + 1] <= 1)

    def evenly_distribute_weekday(self, weekday: int) -> None:
        num_weekdays = num_weekdays_in_time_period(self.start_date, self.num_days, weekday)
        ...
        for days_for_resident in self.day_vars.values():
            ...
            while next_day < self.num_days:
                ...
                next_day += 7
            self.problem.add_constraint(sum(day_of_week_vars) >= min_weekdays_per_resident)
            self.problem.add_constraint(sum(day_of_week_vars) <= max_weekdays_per_resident)
    ...
    def minimize_q2s(self) -> None:
        ...
        for resident, days_for_resident in self.day_vars.items():
            for i in range(len(days_for_resident) - 2):
                ...
                self.problem.add_constraint(0.5 * days_for_resident[i] + 0.5 * days_for_resident[i + 2] == var + var_slack)
                q2s.append(var)
    ...
    def solve(self) -> Solution | str:
        ...
        return Solution(solution.get_objective_value(), solution.get_variables(), self.start_date, self.num_days, self.resident_availability.keys())
