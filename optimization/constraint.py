...
class DistributeDayOfWeekConstraint(Constraint):
    ...
    def get_constraints(self, builder: CallProblemBuilder) -> list[pulp.LpConstraint]:
        ...
        min_weekdays_per_resident = math.floor(num_weekdays / float(builder.get_num_residents()))
        max_weekdays_per_resident = math.ceil(num_weekdays / float(builder.get_num_residents()))
...
class DistributeWeekendsConstraint(Constraint):
    ...
    def get_constraints(self, builder: CallProblemBuilder) -> list[pulp.LpConstraint]:
        ...
        min_per_resident = math.floor(num_weekend_days / float(builder.get_num_residents()))
        max_per_resident = math.ceil(num_weekend_days / float(builder.get_num_residents()))

        first_saturday = days_until_next_weekday(builder.get_start_date(), Weekday.SATURDAY)
...
class LimitWeekdayForResidentConstraint(Constraint):
    ...
    @override
    def deserialize(data: dict[str, Any]) -> Constraint:
        return LimitWeekdayForResidentConstraint(Weekday(data["weekday"]), int(data["limit"]), str(data["resident"]))

    @override
    def serialize(self) -> dict[str, Any]:
        return {"weekday": int(self.weekday), "limit": self.limit, "resident": self.resident}
...
class SetMinimumForDaysOfWeekForResidentConstraint(Constraint):
    ...
    def deserialize(data: dict[str, Any]) -> Constraint:
        ...
        return SetMinimumForDaysOfWeekForResidentConstraint(weekdays, int(data["minimum"]), str(data["resident"]))

    @override
    def serialize(self) -> dict[str, Any]:
        weekdays = ",".join([str(w) for w in self.weekdays])
        return {"weekdays": weekdays, "minimum": self.minimum, "resident": self.resident}

    @override
    def get_constraints(self, builder: CallProblemBuilder) -> list[pulp.LpConstraint]:
        day_varss = [builder.get_vars_for_weekday(self.resident, weekday) for weekday in self.weekdays]
        ...
        for days_for_resident in builder.get_day_vars().values():
            ...
            while next_saturday < builder.get_num_days():
                ...
                if next_saturday + 1 < builder.get_num_days():
                    ...
                    constraints.append(last_saturday + last_sunday + curr_saturday + curr_sunday <= 1)
...
class DistributeQ2sConstraint(Constraint):
    ...
    def get_constraints(self, builder: CallProblemBuilder) -> list[pulp.LpConstraint]:
        ...
        max_q2s = builder.get_problem().max_of(q2s_per_resident, builder.get_num_days(), "max_q2s")
        min_q2s = builder.get_problem().min_of(q2s_per_resident, builder.get_num_days(), "min_q2s")
...
class ConstraintRegistry:
    def __init__(self) -> None:
        self.constraints = {
            c.get_name(): c for c in [DistributeDayOfWeekConstraint, DistributeWeekendsConstraint, LimitWeekdayConstraint, LimitWeekdayForResidentConstraint, SetMinimumForDaysOfWeekForResidentConstraint, NoAdjacentWeekendsConstraint, LimitForPGYConstraint, LimitVACoverageConstraint, DistributeQ2sConstraint, LimitQ2sConstraint, LimitTotalQ2sConstraint]
        }
